...
<div class="back-btn">
    <a href="index.php?action=post&id=<?= ($post->getID()) ?>" class="btn-back-posts offset-md-3"><i class="far fa-chevron-left"></i> Retourner sur la page de l'article</a>
    <hr class="col-md-6 mx-auto">
</div>

<div class="form-add-post">
    <form action="index.php?action=editPost&id=<?= ($post->getID()) ?>" method="POST" enctype="multipart/form-data" class="col-md-6 mx-auto">
        <?php if (isset($error)) { ?>
        ...
        <div class="row">
            <div class="form-group col-lg-12">
                ...
                <input type="text" class="form-control" name="author" id="form-post-author" value="<?= ($post->getAuthor()) ?>" required>
            </div>
        </div>
        <div class="row">
            <div class="form-group col-lg-12">
                <label for="form-post-title">Titre de l'article*</label>
                <input type="text" class="form-control" name="title" id="form-post-title" value="<?= ($post->getTitle()) ?>" required>
            </div>
        </div>
        <div class="form-group">
            <label for="form-post-content">Contenu de l'article*</label>
            <textarea class="form-control" name="content" id="form-post-content" required><?= ($post->getContent()) ?></textarea>
        </div>
        <div class="form-group">
